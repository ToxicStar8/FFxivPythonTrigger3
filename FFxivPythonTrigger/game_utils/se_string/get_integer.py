...
def get_integer(raw_data: bytearray) -> int:
    ...
    if marker < BYTE_LENGTH_CUT_OFF: return marker - 1
    marker = (marker + 1) & 0b1111
    return int.from_bytes((raw_data.pop(0) if marker & (1 << i) else 0 for i in r), signed=True ,byteorder='big')


def make_integer(value: int) -> bytearray:
    if -1 < value < BYTE_LENGTH_CUT_OFF - 1:
        return bytearray([value + 1])
    ret = bytearray([0xf0])
    d = value.to_bytes(4, signed=True, byteorder='little')
    for i in r:
        if d[i] != 0:
            ret.append(d[i])
            ret[0] |= 1 << i
