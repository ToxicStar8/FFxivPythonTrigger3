...
class MapPositionLink(MessageBase):
    ...
    def __init__(self, territory_type_id, map_id, in_game_pos=None, raw_pos=None, map_pos=None):
        self.territory_type_id = territory_type_id
        self.map_id = map_id
        if in_game_pos is not None:
            self.x, self.y = in_game_pos
            self.raw_x = in_game_to_raw_coord(self.x)
            self.raw_y = in_game_to_raw_coord(self.y)
            game_map = map_sheet[self.map_id]
            self.map_x = in_game_to_map_coord(self.x, game_map["SizeFactor"], game_map["Offset{X}"])
            self.map_y = in_game_to_map_coord(self.y, game_map["SizeFactor"], game_map["Offset{Y}"])
        elif raw_pos is not None:
            self.raw_x, self.raw_y = raw_pos
            self.x = raw_to_in_game_coord(self.raw_x)
            self.y = raw_to_in_game_coord(self.raw_y)
            game_map = map_sheet[self.map_id]
            self.map_x = in_game_to_map_coord(self.x, game_map["SizeFactor"], game_map["Offset{X}"])
            self.map_y = in_game_to_map_coord(self.y, game_map["SizeFactor"], game_map["Offset{Y}"])
        elif map_pos is not None:
            self.map_x, self.map_y = map_pos
            game_map = map_sheet[self.map_id]
            self.x = map_to_in_game_coord(self.map_x, game_map["SizeFactor"], game_map["Offset{X}"])
            self.y = map_to_in_game_coord(self.map_y, game_map["SizeFactor"], game_map["Offset{Y}"])
            self.raw_x = in_game_to_raw_coord(self.x)
            self.raw_y = in_game_to_raw_coord(self.y)
        else:
            raise ValueError("Must specify either in-game position, raw position, or map position")
    ...
    def from_buffer(cls, raw: bytearray):
        ...
        return cls(territory_type_id, map_id, raw_pos=(raw_x, raw_y))
